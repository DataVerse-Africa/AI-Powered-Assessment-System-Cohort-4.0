...
class ChatMessage(Base):
    ...
    session_id = Column(
        ...
        ForeignKey('clinical_sessions.id'),
        nullable=False
        # Every message belongs to a specific clinical session.
        # The session links the conversation to the patient and predictions.
    )
    ...
    user_id = Column(
        ...
        ForeignKey('users.id'),
        nullable=False
        # The clinician who initiated this conversation.
    )

    role = Column(
        String(20),
        nullable=False
        # 'user'      -> message sent by the clinician
        # 'assistant' -> response generated by the Groq LLM
    )

    content = Column(
        Text,
        nullable=False
        # The actual message text — no length limit.
    )
